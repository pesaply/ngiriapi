...
ngiriapi includes a `Request` class that gives you a nicer interface onto
...
When you call `async with request.form() as form` you receive a `ngiriapi.datastructures.FormData` which is an immutable
multidict, containing both file uploads and text input. File upload items are represented as instances of `ngiriapi.datastructures.UploadFile`.
...
!!! info
    ...
    ngiriapi to differentiate which data should be treated as file. If `filename` field was supplied, `UploadFile` 
    object will be created to access underlying file, otherwise form-data part will be parsed and available as a raw 
    string.

#### Application

The originating ngiriapi application can be accessed via `request.app`.
