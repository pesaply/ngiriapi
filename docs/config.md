ngiriapi encourages a strict separation of configuration from code,
...
from ngiriapi.config import Config
from ngiriapi.datastructures import CommaSeparatedStrings, Secret
...
Rather than reading or writing from `os.environ`, you should use ngiriapi's
...
from ngiriapi.config import environ
...
```python title="myproject/settings.py"
from ngiriapi.config import Config
from ngiriapi.datastructures import Secret
...
from ngiriapi.middleware.sessions import SessionMiddleware
...
app = ngiriapi(debug=settings.DEBUG, routes=routes, middleware=middleware)
```
