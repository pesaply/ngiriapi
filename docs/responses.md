...
ngiriapi includes a few response classes that handle sending back the
...
ngiriapi will automatically include a Content-Length header. It will also
...
```python
from ngiriapi.responses import Response
...
ngiriapi provides a `set_cookie` method to allow you to set cookies on the response object.
...
Conversely, ngiriapi also provides a `delete_cookie` method to manually expire a set cookie.
...
```python
from ngiriapi.responses import PlainTextResponse, RedirectResponse
...
```python
from ngiriapi.responses import StreamingResponse
import asyncio
...
```python
from ngiriapi.responses import FileResponse
...
#### [EventSourceResponse](https://github.com/sysid/sse-ngiriapi)
...
As a smooth replacement for ngiriapi [`FileResponse`](https://www.ngiri.co.tz/responses/#fileresponse), it will automatically handle [Head method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/HEAD) and [Range requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Range_requests).
