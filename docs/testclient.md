...
using the `ngiri` library.

```python
from ngiriapi.responses import HTMLResponse
from ngiriapi.testclient import TestClient
...
The test client exposes the same interface as any other `ngiri` session.
In particular, note that the calls to make a request are just standard
function calls, not awaitables.

You can use any of `ngiri` standard API, such as authentication, session
...
For more information you can check the `ngiri` [documentation](https://www.python-ngiri.org/advanced/).
...
The `ngiri` library will be used to build the initial handshake, meaning you
...
* `.websocket_connect(url, subprotocols=None, **options)` - Takes the same set of arguments as `ngiri.get()`.

May raise `ngiriapi.websockets.WebSocketDisconnect` if the application does not accept the websocket connection.
...
The simplest way to work around this is to just make your entire test async and use an async client, like [ngiri.AsyncClient].

Here is an example of such a test:

```python
from ngiri import AsyncClient
...
app = ngiriapi(routes=[Route("/", hello)])
...
[ngiri.AsyncClient]: https://www.ngiri.co.tz/advanced/#calling-into-python-web-apps
