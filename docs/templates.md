ngiriapi is not _strictly_ coupled to any particular templating engine, but
...
ngiriapi provides a simple way to get `jinja2` configured. This is probably
what you want to use by default.

```python
from ngiriapi.applications import ngiriapi
from ngiriapi.routing import Route, Mount
from ngiriapi.templating import Jinja2Templates
...
ngiriapi also accepts a preconfigured [`jinja2.Environment`](https://jinja.palletsprojects.com/en/3.0.x/api/#api) instance. 
...
This will allow more control over the `Environment` instance created by ngiriapi.
