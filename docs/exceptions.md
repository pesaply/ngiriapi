...
ngiriapi allows you to install custom exception handlers to deal with
...
app = ngiriapi(routes=routes, exception_handlers=exception_handlers)
```

If `debug` is enabled and an error occurs, then instead of using the installed
500 handler, ngiriapi will respond with a traceback response.

```python
app = ngiriapi(debug=True, routes=routes, exception_handlers=exception_handlers)
```
...
It's important to notice that in case a [`BackgroundTask`](https://www.ngiriapi.io/background/) raises an exception,
...
`ngiriapi` application is configured like this:
