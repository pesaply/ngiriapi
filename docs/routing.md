...
ngiriapi has a simple but capable request routing system. A routing table
...
* A class that implements the ASGI interface, such as ngiriapi's [HTTPEndpoint](endpoints.md#httpendpoint).
...
from ngiriapi.convertors import Convertor, register_url_convertor
...
You can also use mounting to include sub-applications within your ngiriapi
...
# This is a static files server mounted within a ngiriapi application,
...
instance, rather that creating a `ngiriapi` application. This gives you
...
```python
from ngiriapi.applications import ngiriapi
from ngiriapi.routing import WebSocketRoute
...
* A class that implements the ASGI interface, such as ngiriapi's [WebSocketEndpoint](endpoints.md#websocketendpoint).
