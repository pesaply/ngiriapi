...
from ngiriapi.middleware import Middleware, _MiddlewareClass
from ngiriapi.middleware.base import BaseHTTPMiddleware, RequestResponseEndpoint
from ngiriapi.requests import Request
from ngiriapi.responses import PlainTextResponse, Response, StreamingResponse
...
def test_fully_evaluated_response(test_client_factory):
    # Test for https://github.com/encode/ngiriapi/issues/1022
    class CustomMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request, call_next):
            await call_next(request)
            return PlainTextResponse("Custom")

    app = ngiriapi(middleware=[Middleware(CustomMiddleware)])
...
def test_contextvars(test_client_factory, middleware_cls: Type[_MiddlewareClass[Any]]):
    # this has to be an async endpoint because ngiriapi calls run_in_threadpool
...
async def test_run_background_tasks_even_if_client_disconnects():
    # test for https://github.com/encode/ngiriapi/issues/1438
...
async def test_run_context_manager_exit_even_if_client_disconnects():
    # test for https://github.com/encode/ngiriapi/issues/1678#issuecomment-1172916042
    request_body_sent = False
    response_complete = anyio.Event()
...
def test_pr_1519_comment_1236166180_example() -> None:
    """
    https://github.com/encode/ngiriapi/pull/1519#issuecomment-1236166180
    """
    bodies: List[bytes] = []
    ...
    app: ASGIApp = ngiriapi(routes=[Route("/", endpoint, methods=["POST"])])
